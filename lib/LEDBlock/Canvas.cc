...
void Canvas::update() {
  ...
  for (auto const &i : apiobj.parseRules) {
    ...
    // i.second contains an array of size 4 ([lengthOfValue, xPos, yPos, isNumber])
...
void Canvas::setAPI(String name, String url, long refresh, std::map<String, std::array<int, 4>> parseRules) {
...
void Canvas::updateAPI(long curtime) {
    ...
    if (httpcode > 0) {
      ...
      std::map<String, std::array<int, 4>> rules = apiobj.parseRules;
